...
export default function useDashboardStats() {
  const [stats, setStats] = useState({ activeRentals: 0, monthRevenue: 0, totalClients: 0, todayEvents: 0 });
  ...
  useEffect(() => {
    async function fetchStats() {
      ...
      const [activeRes, revenueRes, clientsRes, todayRes] = await Promise.all([
