...
export default async function generateAgreementPdfPO(reservation, items, client, contractData = null, invoiceData = null) {
  ...
  (items || []).forEach((it) => {
    ...
    for (let i = 0; i < qty; i++) {
      eq.push([it.equipment?.name || it.custom_name || it.name || '—', serials[i] || '', rateUnitLabel(it.equipment?.rate_unit || it.custom_rate_unit), fmtPrice(it.daily_rate)]);
    }
  });
  autoTable(doc, {
    startY: y,
  ...
  // With a long equipment list the signature block (OVERENIE + PROTOKOL) starts
