...
| Priority | Task | Notes |
|----------|------|-------|
| **SEO-1** | **Overiť Vercel dev/staging build (SEO, session 43)** | SEO prerender je na `dev` (`4a7b8c2`), zatiaľ **neoverené na Verceli**. Skontrolovať: (a) dev deploy build **prešiel** (`@sparticuz/chromium` musí bežať v Vercel build image — jediná lokálne neoveriteľná časť; ak zlyhá, pozri log a prípadne pinnúť verzie / `PRERENDER_PRODUCTS=0`); (b) fetch staging `/sluzby` → **HTML nesie page-specific `<title>`** (dôkaz, že prerender + filesystem match funguje); (c) `/sitemap.xml` = ~164 URL bez `royal-fleet`/hidden/accessory. Rollback ak treba: `npm run build:spa` alebo env `PRERENDER=0`. |
...
| **SEO-3** | **Merge SEO `dev`→`main` → produkcia** | Po overení SEO-1. Nasadí prerender + robots fix + sitemap na `royalstroje.sk`. |
