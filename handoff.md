...
| 38 | 2026-07-14 | Katalóg: nové príslušenstvo + podkategória „Voľný čas a šport" + 2 elektrobicykle → **PRODUKČNÝ DEPLOY** | **Príslušenstvo:** nový riadok „Pílový list – Alligator 430mm" (15 €/18,45 € s DPH) do Malé náradie → Príslušenstvo (len DB, existujúca podkat.). **Nová podkategória „Voľný čas a šport"** pod Záhradnou technikou (`categories.js` chip + `equipment_subcategories` riadok, sort 50) + **2 elektrobicykle „Green Bike – čierny/biely"** (40 €/49,20 € s DPH, `Bike_black/white.webp`). **Zistenie:** kategórie/podkat. strom je **staticky v `src/data/categories.js`** (sidebar+chipy), produkty žijú v Supabase `equipment` → nová podkategória potrebuje **oboje** (kód+deploy AND DB); nové príslušenstvo do existujúcej podkat. je len DB. **RLS blokuje zápis anon kľúčom** → INSERTy spúšťal owner v Supabase SQL Editore. Commit `3d1daf3` na `dev`, **merge `dev`→`main` (ff) → produkcia**. |
...
2. **New subcategory „Voľný čas a šport"** under Záhradná technika — `categories.js` chip (`volny-cas-a-sport`) + `equipment_subcategories` row (`category_id` 781d3973…, `sort_order` 50).
