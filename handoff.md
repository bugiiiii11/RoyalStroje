...
| 37 | 2026-07-03 | Hero fotky + blog vlastné hero obrázky + drobný polish → **PRODUKČNÝ DEPLOY** | **Hero pás:** ľavá fotka `dovoz.webp` (auto) → nová `web_pics/doprava1.webp` (JCB 19C-1 na vlečke). **Blog:** každý článok má v hero sekcii svoj vlastný obrázok (`image` pole v `blogMeta`, `BlogDetail` berie `meta.image` namiesto zdieľaného `/hero-pozicovna.webp`; všetkých 19 zapojených). **Hero polish:** menšie nadpisy (clamp 1.55/2.3vw/2.6→1.35/2vw/2.2), biely oblačik pri „Predaj" len okolo textu (tlačidlo von), CTA „Navštíviť predajňu" na spodok (zarovnané s ľavým CTA), OOPP → „osobné ochranné pracovné prostriedky" + zalomené na 2 riadky. **Kontakt galéria:** +`pasovy_bager.webp` +`strojovy_park.webp` (7 fotiek). **MERGE `dev`→`main` (fast-forward, 38 commitov `f0865f7`→`60d874f`) → nasadené na produkciu royalstroje.sk.** Commity `3506586`→`60d874f`. |
...
| 42 | 2026-07-16 | Katalóg: nový dekoračný bager (bager_web) vľavo hore → **PRODUKČNÝ DEPLOY** | Owner render JCB 19c (`bager_web.png`, RGBA s priehľadnosťou) → WebP 1024×683 q80 (177 KB, z 2,79 MB), zdroj presunutý do `web_pics/`, `Catalog.jsx` swap ľavého rohového stroja; merge `dev`→`main` (ff, `cf4e54f`) → produkcia |
...
| 44 | 2026-07-16 | Fix „bliknutia" prerendered stránok pri boote + SEO-1 overené na stagingu | Prerendered HTML sa pri boote „bliklo" (React `createRoot().render()` nahradil DOM → reštart hero animácií + js-reveal re-hide). Fix: `html[data-prerendered]` marker v snapshotoch, inline skript preskočí js-reveal, HeroSplit potlačí vstupné animácie, App.jsx po prvej navigácii obnoví normálne správanie. Puppeteer verifikácia 12/12. **SEO-1 KOMPLET overené na Vercel stagingu** (chromium build OK, per-page titles, sitemap 164). Commit `92c571d` na `dev`. |
| 43 | 2026-07-16 | SEO: prerender celého webu + build-time sitemap + noindex/meta fixy (na `dev`, NEnasadené) | **Root cause slabej indexácie (~3 stránky):** `robots.txt` mal `Disallow: /assets/` → Googlebot si nemohol stiahnuť JS bundle → každá URL = prázdny shell. **Fixy:** (1) odblokovaný `/assets/` + `*.json`; (2) **post-build Puppeteer prerender ~177 URL** (statické + 19 blog + 143 produktov zo Supabase) do `dist/<path>/index.html` — Vercel servuje hotové HTML, SPA rewrite ostáva fallback; (3) **build-time sitemap** (164 URL, nahradila zastaranú ručnú s mŕtvou royal-fleet); (4) **fix react-helmet-async v3 bug** — `<title>` s viac JSX detmi ostal PRÁZDNY (ProductDetail+BlogDetail aj v prod) → template literal; (5) **noindex** na 9 skrytých blog článkov (nový `blogMeta.js` single source of truth), NotFound stránka namiesto soft-404 redirectu, catch-all `*` route; (6) meta diery: Partneri Helmet, ProductDetail canonical, app-level og defaults, blog og:image; (7) `vercel.json trailingSlash:false`. Build overený lokálne (177 stránok, dedupe title/desc/og OK). Commit `4a7b8c2` na `dev`. ⏳ Čaká: overiť Vercel staging build (chromium), owner www→apex redirect, merge→prod, GSC sitemap. |
...
## What Was Done (Session 37) -- Hero fotky + blog vlastné hero obrázky + drobný polish → PRODUKČNÝ DEPLOY
Date: 2026-07-03
**Status: SHIPPED TO PRODUCTION.** All session 24–37 work merged `dev`→`main` (fast-forward, 38 commits, `f0865f7`→`60d874f`) and pushed → Vercel auto-deploys royalstroje.sk. `main` == `dev`. Owner: continue editing on `dev`; each future `dev`→`main` merge ships the next batch.

### Website
1. **New hero-strip image (doprava1)** -- the left photo in the HeroSplit 4-image strip (under the "Dovoz techniky" USP) swapped from `dovoz.webp` (Ford van) to owner's new `web_pics/doprava1.webp` (JCB 19C-1 on a Royal Stroje trailer). jpg→webp, downscaled 1280→1000px q80 (311→135 KB). Files: `src/components/home/HeroSplit.jsx`, `public/pictures/graphics/web_pics/doprava1.{jpg,webp}`. Committed: `3506586`.
2. **Per-article blog hero images** -- `BlogDetail` used one shared `/hero-pozicovna.webp` for every article's `PageHero`. Added an `image` field to every entry in `blogMeta` (the same preview image shown on the Blog listing) and wired `PageHero image={meta.image || '/hero-pozicovna.webp'}`. All 19 entries populated (10 released + 9 hidden — future-proof). Note: `blogMeta.image` now duplicates the `image` in `Blog.jsx`'s inline `blogPosts` array (pre-existing duplication pattern; kept in sync manually). Files: `src/data/blogArticles.jsx`, `src/pages/BlogDetail.jsx`. Committed: `3506586`.
3. **Hero headings + cloud + CTA polish** -- (a) both split headings shrunk `text-[clamp(1.55rem,2.3vw,2.6rem)]`→`clamp(1.35rem,2vw,2.2rem)`. (b) The "Predaj náradia" white "cloud" now backs only the text — the "Navštíviť predajňu" button moved outside the cloud wrapper (desktop + mobile), keeping its `.btn-primary` hover-lift (entrance animation stays on the wrapper, not the button). (c) Right column → `justify-between` so the CTA drops to the bottom, aligned with the left "Zavolať teraz" CTA. Files: `src/components/home/HeroSplit.jsx`. Committed: `eb952c9`, `83b7b7c`.
4. **OOPP spelled out** -- "OOPP" → "osobné ochranné pracovné prostriedky" in the two visible Predaj subtitles + both image alts (genitive in alt); then a hard `<br />` so "a osobné ochranné pracovné prostriedky" sits on line 2 (desktop only). The blog article with its own OOPP definition (`bezpecnostne-pravidla-stavebne-stroje`) left untouched. Files: `src/components/home/HeroSplit.jsx`. Committed: `83b7b7c`, `60d874f`.
5. **Kontakt gallery +2 photos** -- added `pasovy_bager.webp` (JCB on trailer; jpg→webp 1600px q80, 575→245 KB) and `strojovy_park.webp` (machine yard; jpg→webp 1280px q66, 270→242 KB — lowered q so webp < source on the detail-heavy photo) to `GALLERY_IMAGES` (now 7). They flow into the CSS-columns masonry grid + lightbox automatically (counter `N/7`, prev/next wrap via `.length`). Files: `src/pages/Kontakt.jsx`, `public/pictures/graphics/web_pics/{pasovy_bager,strojovy_park}.{jpg,webp}`. Committed: `83b7b7c`, `60d874f`.

### Deploy
6. **`dev`→`main` fast-forward merge → production** -- `main` was 38 commits behind (all of sessions 24–37); merged with `--ff-only` (clean linear history, no merge commit), pushed `f0865f7`→`60d874f`. Vercel auto-deploys `main` → royalstroje.sk. Switched back to `dev` after. This shipped the **entire multi-session redesign** to production: light dark-on-light theme site-wide, new `HeroSplit`, the "Zoženieme akýkoľvek stroj" service + homepage sourcing banner, Source Sans 3 body font, ProductDetail white-hero redesign, all 19 blog articles re-themed, and the session 33/36 UX polish.

### Verified
- `npm run build` clean before each push (~9–12 s). Branch relationship checked before merge (`origin/main` is an ancestor of `dev`; 0 commits on `main` not on `dev` → true fast-forward). Image conversions kept within the existing gallery size band (163–275 KB). GPU + reveal guards untouched (no new `backdrop-filter` on fixed/sticky; no reveal end-state changes).
- **Not device-tested on real budget Android** (standing heads-up) — but the redesign is now on production, so watch for any real-hardware GPU issues (see [[mobile-gpu-garbage-bug]]).

## What Was Done (Session 38) -- Katalóg: príslušenstvo + podkategória „Voľný čas a šport" + 2 elektrobicykle
Date: 2026-07-14
**Status: SHIPPED TO PRODUCTION.** Commit `3d1daf3` on `dev`, fast-forward merged `dev`→`main` + pushed → Vercel auto-deploys royalstroje.sk. `main` == `dev`. DB inserts run by owner in Supabase SQL Editor (RLS blocks the repo's anon key).

### Katalóg — data model recap (important)
- The category → subcategory **tree is STATIC in `src/data/categories.js`** (drives the catalog sidebar + subcategory chips; `Catalog.jsx` imports `categories`). **Products** live in Supabase `equipment`, matched to the tree by `category` + `subcategory` **slug**.
- So: adding a product into an **existing** subcategory = **DB-only** (no deploy). Adding a **new subcategory** = needs **BOTH** a `categories.js` entry (code → commit + deploy) **AND** an `equipment_subcategories` row (DB), else the chip won't render (products would only show under „Všetko").
- **RLS blocks writes with the repo's anon key** (verified) — all `equipment` / `equipment_subcategories` INSERTs must be run by the owner in the Supabase SQL Editor (or via the authenticated dashboard `EquipmentForm`). No service-role key is in the repo.
- **Image paths:** `image_path` stores a legacy public path with spaces + diacritics directly, e.g. `/pictures/Katalog-PNG/Voľný čas a šport/Bike_black.webp` (browser encodes at render). Folder is `Katalog-PNG` (capital K) — **case-sensitive on Vercel/Linux**. Image files live in `public/` → must be committed + deployed to appear (unlike DB rows, which are live immediately on both sites).
- **Supabase project (from `.env`, source of truth):** `kvosclcbykjpsmqzdrjs.supabase.co`. (The old „Supabase" section below lists `dvmdoczuppmcumykhktm` — outdated; trust `.env`.)

### What was added
1. **New accessory „Pílový list – Alligator 430mm"** — 15 € bez DPH (18,45 € s DPH) into Malé náradie → Príslušenstvo. **DB-only** (existing subcategory). Name uses the `" - "` convention so the accessories table splits it into Produkt/Parametre columns. `slug` `accessory-16-pilovy-list-alligator`, `features: ["Alligator 430mm"]`, `pricing_type fixed`.
2. **New top-level category „Voľný čas a šport"** (sidebar, at the very bottom, `sort_order` 70) — `categories.js` new top-level object (`volny-cas-a-sport`, subcategories = just „Všetko") + `equipment_categories` row (`ownership_type` owned). ⚠ **First shipped by mistake as a *subcategory* under Záhradná technika** (chip); owner flagged it. Corrected: removed the subcategory from `categories.js`, added the top-level category; SQL moved bikes (`UPDATE equipment SET category_id = <new>, subcategory_id = NULL`) and deleted the stray `equipment_subcategories` row. Lesson: „nová kategória dole v sidebar" = top-level `equipment_categories` + top-level object in `categories.js`, NOT a subcategory.
3. **2 e-bikes „Green Bike – čierny / – biely"** — 40 € bez DPH (49,20 € s DPH) each, render as ProductCards. Images `public/pictures/Katalog-PNG/Voľný čas a šport/Bike_{black,white}.webp` (owner-supplied WebP, 1024×768, 34/73 KB). Shared `features` (Dojazd až 160 km, rýchlosť 25–41 km/h, hmotnosť 21 kg, nabíjanie 7–8,5 h). Slugs `green-bike-cierny` / `green-bike-biely`. Subcategory kept (NOT NULL constraint on `equipment.subcategory_id` → can't null it) — the stray subcategory was moved under the new category + renamed to „Bicykle" (slug `bicykle`); no chip in `categories.js` (bikes show under „Všetko"). Bikes stayed via `UPDATE equipment SET category_id = <new>`.
4. **Sidebar icon for „Voľný čas a šport"** — added `Bike` (lucide-react) to `categoryIcons` map in `Catalog.jsx` (keyed by category id `volny-cas-a-sport`); category icons are a static slug→icon map there, NOT in `categories.js`.
5. **Removed „Nezáväzná ponuka" CTA from product detail** — the secondary `<Link to="/cenova-ponuka">` button in the price-card CTAs on `ProductDetail.jsx` (shown on every product) removed at owner's request; only „Zavolať 0948 555 551" primary CTA remains. Dropped now-unused `ArrowRight` import. (Catalog `ProductCard` never had this button — it has Zobraziť detail / Zavolať.)

### Files
- `src/data/categories.js` — added `{ id/name/slug: 'volny-cas-a-sport' … }` under `zahradna-technika`.
- `public/pictures/Katalog-PNG/Voľný čas a šport/Bike_black.webp` + `Bike_white.webp` — new.
- SQL (run in Supabase, not in repo): 1× `equipment_subcategories` INSERT + 3× `equipment` INSERT (1 accessory + 2 bikes).

### Verified
- `npm run build` clean (17 s; only the pre-existing 500 kB chunk-size warning). DB inserts confirmed via anon read: subcategory + both bikes `active`, correct paths, 40 €. Fast-forward merge confirmed (`origin/main` ancestor of `dev`; 0 commits on `main` not on `dev`).
- **Not device-tested on real budget Android** (standing heads-up).

## What Was Done (Session 39) -- Hero vstupné animácie + fix prázdneho katalógu na back-navigácii
Date: 2026-07-14
**Status: SHIPPED TO PRODUCTION.** Commit `b631398` on `dev`, fast-forward merged `dev`→`main` (`15240f1`→`b631398`) + pushed → Vercel auto-deploys royalstroje.sk. `main` == `dev`.

### Hero (HeroSplit) — vstupné animácie
1. **Dva veľké obrázky splitu + 4 USP karty dostali entrance animáciu** — predtým animovali len textové bloky (`hs-0`–`hs-4`) a 4-obrázkový pás; ľavé auto, pravé náradie a spodné USP karty sa zobrazovali okamžite (pôsobilo nedokončene). Pridané: `.hs-img-l` (auto — fade + nábeh zľava, drží `translateY(-50%)` kvôli centrovaniu), `.hs-img-r` (náradie + mobilné náradie — fade + jemné doostrenie zo `scale(1.06)`, končí na `transform: none`), USP karty `.hs-card` so staggerom (`animationDelay` 0.48–0.72 s inline), mobilné auto reuse `.hs-in`. Poradie: obrázky ako pozadie prvé → texty → pás → USP karty. Files: `src/components/home/HeroSplit.jsx`. Committed: `b631398`.
2. **Pás obrázkov presunutý z inline `animation` na triedu `.hs-in`** (delay ostáva inline) — aby ho pokryl nový `prefers-reduced-motion` guard. **Pridaný `@media (prefers-reduced-motion: reduce)`** — všetky hero animácie (`hs-0..4`, `hs-in`, `hs-card`, `hs-img-l/r`) sa vypnú (`animation: none`, `opacity: 1`); pozor: `.hs-img-l` sa v guarde NEnuluje transform (Tailwind `-translate-y-1/2` drží centrovanie). GPU guard dodržaný (žiadny `backdrop-filter` na fixed/sticky; transform-animácie končia na `none` okrem centrovaného ľavého auta — desktop-only, nie scroll-trigger). Files: `src/components/home/HeroSplit.jsx`. Committed: `b631398`.

### Fix — kategória „Voľný čas a šport" prázdna po návrate z detailu produktu
3. **`useProducts` initial state teraz použije modulový cache namiesto `staticProducts`** — root cause: hook sa pri KAŽDOM mount-e inicializoval na `staticProducts` (SSR-safe fallback), ktoré obsahujú produkty ostatných kategórií, ale **NIE bicykle** (tie žijú len v Supabase `equipment`). Klik na bicykel → detail → „Späť" (`navigate(-1)`) remountne `Catalog` s `products = staticProducts` → „Voľný čas a šport" prázdna, kým nedobehne Supabase fetch. Ostatné kategórie ten flash nemajú (ich produkty v `staticProducts` sú). Oprava: `useState(cachedProducts || staticProducts)` + `useState(!cachedProducts)` pre loading — keďže bicykle museli byť raz načítané (aby na ne klik), `cachedProducts` je populated a návrat ich má hneď v úvodnom stave. Bonus: odstraňuje krátky empty-flash aj pre všetky ostatné kategórie. Files: `src/hooks/useProducts.js`. Committed: `b631398`.

### Verified
- `npm run build` clean (~10 s; len pre-existing 500 kB chunk-size warning). ESLint čistý na oboch zmenených súboroch.
- **Not device-tested on real budget Android** (standing heads-up). Odporúčané po deployi manuálne prekliknúť: hero load + scenár bicykel → detail → späť.

## What Was Done (Session 40) -- Zmluva PDF fix (4+ položiek) + „Späť" v Súhrne obchodu
Date: 2026-07-16
**Status: SHIPPED TO PRODUCTION.** Commity `0ff8b81` (PDF fix) + `f66b089` (Späť tlačidlo) na `dev`, fast-forward `dev`→`main` (`b631398`→`f66b089`) → produkcia (dashboard `app.royalstroje.sk`). `main` == `dev`.

### Zmluva PDF — rozloženie pri 4+ položkách (owner reportoval rozbité rozloženie)
1. **Blok podpisov sa presunie na 2. stranu, ak sa nezmestí** — pri dlhom zozname položiek začínal blok „OVERENIE OPRÁVNENIA A PODPISY"/„PODPISY ZMLUVNÝCH STRÁN" + „PROTOKOL O VRÁTENÍ PP" privysoko a `autoTable` ho rozdelil cez viac strán (hlavička sa repeatla na každej → rozsypané + prázdne strany). Fix: pred vykreslením bloku dynamický fit-check `if (y + SIG_BLOCK_H > pageH - 16) { doc.addPage(); y = 12; }` (`SIG_BLOCK_H = 62`) — celý blok tak ostane pokope na čistej druhej strane. Files: `apps/dashboard/src/lib/generateAgreementPdfPO.js`, `apps/dashboard/src/lib/generateAgreementPdf.js`. Committed: `0ff8b81`.
2. **Číslovanie strán** — po vykreslení bloku: ak `doc.getNumberOfPages() > 1`, na každú stranu vpravo dole `Strana p/N` (`w-M`, `pageH-8`, right-align, fontSize 7, `LBL_C`). Len pri 2-stranovej zmluve; jednostranové ostávajú bez čísla (owner request). Committed: `0ff8b81`.
3. **Kalibrácia MERANÍM, nie odhadom** — throwaway Node harness (esbuild bundle pre FO / `--experimental-loader` so supabase stubom pre PO, jsPDF `API.save` patch na capture doc-u, reálne Inter fonty čítané z `public/fonts/` cez fetch stub) odmeral `sigY` + page-count pre 1–8 položiek: **PO** fituje ≤3 (koniec bloku ~278 mm), zlomí sa na **4** (sigY ~222 mm); **FO** (má navyše NOTES bloky) fituje ≤4, zlomí sa na **5**. Blok = ~62 mm, `autoTable` preteká keď koniec > ~282 mm. Fit-check `y+62 > 281` čisto oddelil všetky prípady. **Prečo height-fit a nie tvrdý „4+" počet:** tvrdý počet by pri FO robil zbytočnú 2. stranu (FO 4 pol. sa ešte zmestí) a nezohľadňoval dlhé poznámky/adresy, ktoré tlačia obsah nižšie; dynamický `y` to rieši samo. Overené aj vykreslenie `Strana 1/2`+`Strana 2/2` (grep PDF streamu s helvetica fallbackom). Platí pre návrh, finálnu zmluvu aj faktúru (FO aj PO). Test súbory zmazané po overení.

### Nový obchod (dashboard) — tlačidlo „Späť" v kroku Súhrn
4. **Krok Súhrn (`step===2`) dostal „Späť"** — dovtedy mal len full-width „Vytvoriť obchod"; zdieľaná Späť/Ďalej navigácia v `NewDeal.jsx` je zámerne vylúčená pre `step===2` (`step < 3 && step !== 2`), lebo `NewDealStepReview` má vlastné CTA. Pridaný `onBack` prop (`() => setStep(1)` → späť na Zariadenia, stav `client/items/dateFrom/…` ostáva v `NewDeal` state) + tlačidlo v riadku (`flex items-center gap-3`) vedľa CTA, ktoré ostáva veľké (`flex-1`). **Späť je svetlozelené** (`bg-green-50 text-green-700 border border-green-300 hover:bg-green-100`) pre lepšiu viditeľnosť (owner request) oproti sivému outline v kroku 2; disabled počas `submitting`. Files: `apps/dashboard/src/pages/deals/NewDealStepReview.jsx`, `apps/dashboard/src/pages/deals/NewDeal.jsx`. Committed: `f66b089`.
...
## What Was Done (Session 42) -- Katalóg: nový dekoračný bager (bager_web) → PRODUKČNÝ DEPLOY
Date: 2026-07-16
**Status: COMMITTED + PUSHED + PRODUKČNÝ DEPLOY** (`dev`→`main` fast-forward).

Owner dodal nový render JCB 19c (`public/bager_web.png`) a chcel ním nahradiť ľavý rohový dekoračný stroj v katalógu (predtým `bager.webp`).

1. **PNG → WebP (s alfou)** -- owner PNG bol **1536×1024 RGBA s reálnou priehľadnosťou** (58 % pixelov plne transparentných + poloprehľadný prašný glow; tmavé pozadie v náhľade bola len čierna podložka prehliadača, nie zapečená farba — **iné než session-32 PNG-čka, ktoré mali baked-in checkerboard RGB bez alfy**). Skonvertované cez PIL: resize na retina rozmer **1024×683** (zobrazuje sa ~570×380 CSS px), `quality=80 method=6`, alfa zachovaná → **177 KB** (z 2,79 MB). Overené kompozitom na `#FAFAFA` — bager splýva so svetlým katalógom (prašný tieň prechádza do pozadia), žiadny box, žiadne artefakty.
2. **Zdrojový PNG presunutý do konvencie** -- `public/bager_web.png` (root, deployoval by sa zbytočne) → `public/pictures/graphics/web_pics/bager_web.png` (vedľa webp, ako ostatné `*.png`+`*.webp` páry).
3. **Catalog.jsx swap** -- `src` `bager.webp` → `bager_web.webp`, `width/height` `1000×664` → `1024×683` (pomer strán takmer identický 1.50 vs 1.51, takže `max-w-[calc(50vw-356px)] max-h-[380px] top-[20px] left-0` pozícia bez zmeny). Files: `src/components/home/Catalog.jsx:219`.

### Poznámky / heads-up
- Pôvodný `bager.webp` / `bager.png` (session 32) sú teraz **orphany** (nikde nereferencované) — nechané na disku, neškodné; možné zmazať pri budúcom cleanupe (aj s ~1,5 MB zdrojovým PNG). Pravý rohový stroj `auto_katalog.webp` bez zmeny.

### Verified
- `vite build` čistý (16,7 s), ESLint na `Catalog.jsx` čistý. Commit `cf4e54f` na `dev`, **merge `dev`→`main` (ff, `38acb00`→`cf4e54f`) → produkcia royalstroje.sk**.

## What Was Done (Session 43) -- SEO: prerender celého webu + build-time sitemap + noindex/meta fixy
Date: 2026-07-16
**Status: COMMITTED na `dev` (`4a7b8c2`), NEnasadené.** Owner reportoval, že Google indexuje len ~3 stránky. Plán: `.claude/plans/ahoj-chcel-by-som-curious-meadow.md`.

### Diagnóza (dve veľké zistenia)
1. **Root cause indexácie: `robots.txt` blokoval `/assets/`** — Vite tam ukladá VŠETKY JS/CSS bundly. Googlebot renderer si ich nemohol stiahnuť → SPA sa nespustila → každá stránka preňho prázdny shell. Preto ~3 stránky v indexe.
2. **Bug žijúci aj v produkcii:** `react-helmet-async v3` nevyplní `<title>`, keď dostane viac JSX detí (`<title>{a} - {b}</title>`). ProductDetail + BlogDetail mali doteraz **prázdny/default title aj v ostrom nasadení**. Fix = jeden template literal `<title>{`${a} - ${b}`}</title>`.

### Čo sa spravilo
1. **`robots.txt`** — zmazaný `Disallow: /assets/` + `Disallow: /*.json$` (ponechaný `/kosik`). Files: `public/robots.txt`. Najvyšší dopad.
2. **Prerender (`scripts/prerender.mjs`)** — post-build Puppeteer (`puppeteer-core` + `@sparticuz/chromium`) driving `vite preview`, snapshot **~177 URL** (11 statických indexovateľných + 4 noindex legal/kosik + 19 blog + 143 produktov zo Supabase) do `dist/<path>/index.html`. Vercel filesystem-before-rewrites servuje hotové HTML; SPA rewrite ostáva fallback. Sanitácia snapshotu: dedupe `<title>`/`description`/`og:image` (helmet v3 NEznačí tagy `data-rh`), strip `js-reveal` z `<html>`, strip chatbot/recaptcha tagy, cookie consent pre-set. Concurrency 5, ~40 s. Supabase REST cache (177 fetchov → 1). Escape hatch `PRERENDER=0` / `PRERENDER_PRODUCTS=0`.
3. **Build-time sitemap (`scripts/generate-sitemap.mjs` + `scripts/lib/collect-urls.mjs`)** — emituje `dist/sitemap.xml` (**164 URL**): statické + viditeľné blog (lastmod z `dateSort`) + produkty zo Supabase REST (fallback `products.js`). `accessory-*` orphan stránky vylúčené. Zmazaná ručná `public/sitemap.xml` (mala mŕtvu `/sluzby/royal-fleet`, chýbalo ~150 URL).
4. **`package.json` build pipeline** — `build: vite build && node scripts/generate-sitemap.mjs && node scripts/prerender.mjs`; nový `build:spa: vite build` (rollback). +2 dev deps.
5. **`src/data/blogMeta.js` (NOVÝ)** — plain ESM single source of truth (`blogPosts` + odvodené `blogMeta`), `hidden: true` flag na 9 článkoch. Konsolidoval rozchádzajúce sa dáta z `Blog.jsx` (inline pole) a `blogArticles.jsx`. `Blog.jsx` filtruje `!post.hidden`; `blogArticles.jsx` re-exportuje.
6. **Noindex + soft-404 fix** — nový `src/pages/NotFound.jsx` (noindex Helmet); `ProductDetail.jsx` neznámy produkt → `<NotFound/>` (nie redirect na home) + canonical; `BlogDetail.jsx` skrytý článok → `noindex` + bez canonicalu, neznámy slug → `<NotFound/>`; `App.jsx` catch-all `<Route path="*">`.
7. **Meta diery** — `Partneri.jsx` doplnený Helmet (predtým žiadny); `App.jsx` app-level default `<Helmet>` (og:site_name/og:locale/twitter:card/og:image); blog og:image z `meta.image`.
8. **`vercel.json`** — `trailingSlash: false` (dedup `/sluzby/` vs `/sluzby`).

### Verified (lokálne)
- Full build čistý, **177 stránok prerenderovaných**. Overené: každá stránka má práve 1 `<title>` + 1 `description` + správny canonical; produktové/blog stránky majú page-specific title; skryté články majú `noindex`; `<html>` bez `js-reveal`; kosik/legal noindex zapečený; sitemap 164 URL bez royal-fleet/hidden/accessory. Runtime (Puppeteer): `/neexistuje` + `/sluzby/xyz` → NotFound s noindex; `/blog` = 10 článkov; skrytý článok renderuje + noindex.
- ESLint na zmenených súboroch bez nových chýb (repo baseline 601 pre-existing).
- **Neoverené lokálne:** či `@sparticuz/chromium` beží v Vercel build image (jediná vec bez lokálnej repro) — treba skontrolovať dev deploy.

### Files
- Nové: `scripts/prerender.mjs`, `scripts/generate-sitemap.mjs`, `scripts/lib/collect-urls.mjs`, `src/data/blogMeta.js`, `src/pages/NotFound.jsx`.
- Zmenené: `public/robots.txt`, `vercel.json`, `package.json`, `src/App.jsx`, `src/pages/{ProductDetail,BlogDetail,Blog,Partneri}.jsx`, `src/data/blogArticles.jsx`.
...
1. **Prerender boot "blink" fixed** -- Owner reported staging "blinks" on load. Root cause: prerendered HTML paints the full page instantly, then React boots and `createRoot().render()` (NOT hydrate) wipes and remounts `#root` -- HeroSplit entrance animations restarted from opacity 0 and the `index.html` inline script had re-added `js-reveal`, re-hiding all reveal content until IntersectionObserver fired. Fix in 4 parts: (a) `scripts/prerender.mjs` stamps `<html data-prerendered>` into every snapshot; (b) `index.html` inline script skips adding `js-reveal` when the attribute is present (reveal content stays visible through the React remount); (c) `HeroSplit.jsx` suppresses all `hs-*` entrance animations under `html[data-prerendered]` (same `animation: none !important` pattern as its `prefers-reduced-motion` block; `.hs-img-l` centering survives via the Tailwind `-translate-y-1/2` base class); (d) new `RestoreRevealsAfterNav` component in `App.jsx` lifts the attribute + restores `js-reveal` on the FIRST client-side navigation (`useLayoutEffect` on pathname, skips initial render) so scroll reveals + hero animations behave like the plain SPA for the rest of the session. Hydration (`hydrateRoot`) was rejected: IO-driven `in-view` classes + Supabase-fetched catalog data baked into snapshots guarantee mismatches. Verified with a throwaway puppeteer-core harness against built dist (12/12 checks: boot shows hero at opacity 1 / animation none, reveals not re-hidden, first nav restores js-reveal + `.in-view` fires, `build:spa` shell path unchanged). Files: `scripts/prerender.mjs`, `index.html`, `src/components/home/HeroSplit.jsx`, `src/App.jsx`. Committed: `92c571d`, pushed to `dev`.
2. **SEO-1 fully verified on Vercel staging** -- After the push, polled staging until the new deploy went live, then verified over HTTP: (a) build with `@sparticuz/chromium` passes on Vercel (staging serves 147 KB prerendered homepage with `data-prerendered` + suppression CSS baked); (b) `/sluzby` static HTML carries page-specific `<title>` ("Sluzby - Prenajom a predaj stavebnej techniky | Royal Stroje Senec") -- filesystem-before-rewrites works; (c) product page `/atlas-copco-lg400` has full title + canonical (helmet v3 bug stays fixed); (d) `/sitemap.xml` = 164 URLs, no `royal-fleet`. Note: local `vite preview` does NOT do filesystem-before-rewrites for extensionless paths (serves the shell), so per-page titles can only be verified on Vercel or by reading `dist/<path>/index.html` directly.
3. **Console warnings on staging triaged (no action)** -- `contentscript.js` ObjectMultiplex/MaxListeners spam = owner's browser extension, not the site; `Recaptcha script is not available` = known domain restriction on `*.vercel.app` previews (handoff item 0a).

## What To Do Next
| Priority | Task | Notes |
|----------|------|-------|
| **SEO-1** | ✅ **DONE (session 44): Vercel staging build overený** | Chromium build prešiel, staging servuje prerendered HTML s per-page titles (`/sluzby` + produkty + canonical), sitemap 164 URL bez `royal-fleet`. Navyše opravený boot „blink" prerendered stránok (`92c571d`). Pozn.: lokálny `vite preview` NErobí filesystem-before-rewrites — per-page titles over cez Vercel alebo priamo v `dist/<path>/index.html`. |
| **SEO-2** | **OWNER: www → apex redirect** | Google indexuje `www.royalstroje.sk` (duplicita, štiepi signály). Vercel dashboard → projekt `royal-stroje` → **Settings → Domains** → pri `www.royalstroje.sk` nastaviť **Redirect to `royalstroje.sk`** (308). Web-level zmena, nie kód. |
| **SEO-3** | **Merge SEO `dev`→`main` → produkcia** | **ODBLOKOVANÉ** (SEO-1 overené v session 44). Nasadí prerender + robots fix + sitemap + blink fix na `royalstroje.sk`. Ideálne po ownerovom vizuálnom OK stagingu (blink preč?). |
| **SEO-4** | **OWNER (po deployi): Google Search Console** | (1) Sitemaps → odoslať `https://royalstroje.sk/sitemap.xml`. (2) URL Inspection na `/`, `/sluzby`, 1–2 top produkty → „View crawled page" musí ukázať per-page title/meta → **Request indexing**. (3) Sledovať **Pages (Coverage)** report — nárast indexovaných stránok 2–4 týždne. |
| **SEO-5** | (Voliteľné, session 43 Fáza 4) SEO extras | FAQPage JSON-LD na homepage FAQ (`src/components/home/FAQ.jsx`, treba plain-string `answerText`); `sameAs` (GBP/Maps + Facebook URL od ownera) do LocalBusiness schémy v `Home.jsx`. Nice-to-have, nízka priorita. |
| **SEO-6** | (Voliteľné) Prerender freshness | Nový/zmenený produkt v Supabase sa v statickom HTML objaví až po ďalšom deployi (JS verzia je vždy čerstvá). Ak bude vadiť: Vercel Deploy Hook, ktorý dashboard pingne pri zmene produktu. Zatiaľ stačí redeploynut po väčších zmenách katalógu. |
| 0 | ✅ **DONE (session 37): SHIPPED TO PRODUCTION — merged `dev`→`main`** | The whole redesign (sessions 24–**36**) + new "Zoženieme akýkoľvek stroj" service + homepage "Nenašli ste stroj?" banner + hero 4-image strip + the **dark-elements-on-light-bg** theme now across **the ENTIRE site** + new hero truck render + 4 s Akcie banner + session-32 hero image swap & catalog corner machines + session-33 UX/UI polish + session-34 Source Sans 3 body font, catalog cards without availability state, ProductDetail redesign, mobile fixes + session-35 ProductDetail white hero, Archivo word-spacing, hover seam fix, new Blog/Partneri hero photos + **session-36 web-wide polish (thinner eyebrows, no watermark numbers, card-grid variety, catalog hover seam fix) + all 19 blog articles re-themed readable on the light bg** were committed on `dev` and reviewed on the public Vercel preview URL. **✅ Session 37: merged `dev`→`main` (fast-forward, 38 commits `f0865f7`→`60d874f`) → production royalstroje.sk is now LIVE with the full redesign.** Owner: continue editing on `dev`; each future `dev`→`main` merge ships the next batch. |
| 1 | **Delete dead hero files (now unblocked — `main` merge done)** | `src/components/home/Hero.jsx` + `MobileHero.jsx` (and `Hero1.jsx` if present) were kept only for revert while `HeroSplit` stabilized. Production now ships `HeroSplit`, so they're safe to delete along with the commented-out imports + old hero block in `src/pages/Home.jsx`. Verify the files exist first, then remove. Small cleanup. |
...
| File | Purpose |
|------|---------|
| `scripts/prerender.mjs` | **NEW (session 43)** post-build Puppeteer prerender: snapshotuje ~177 URL do `dist/<path>/index.html` (crawler-readable HTML). Sanitácia: dedupe title/desc/og:image (helmet v3 nemá `data-rh`), strip `js-reveal`+widgety, cookie pre-set. `PRERENDER=0`/`PRERENDER_PRODUCTS=0` escape hatch. Lokálne Chrome/Edge, Vercel `@sparticuz/chromium` |
| `scripts/generate-sitemap.mjs` + `scripts/lib/collect-urls.mjs` | **NEW (session 43)** build-time sitemap → `dist/sitemap.xml` (164 URL). `collect-urls.mjs` = zdieľaný inventár (statické + viditeľné blog + Supabase produkty, fallback `products.js`, `accessory-*` vylúčené) pre sitemap aj prerender |
| `src/data/blogMeta.js` | **NEW (session 43)** single source of truth pre blog metadáta (plain ESM, `blogPosts` + odvodené `blogMeta`). `hidden: true` = unlisted + noindex + mimo sitemap. Konzumuje `Blog.jsx`, `BlogDetail.jsx`, Node build skripty. `blogArticles.jsx` re-exportuje |
| `src/pages/NotFound.jsx` | **NEW (session 43)** 404 view s `noindex` Helmetom. Renderuje ProductDetail (neznámy produkt), BlogDetail (neznámy slug), App `*` catch-all — namiesto starého soft-404 redirectu na homepage |
| `public/robots.txt` | **UPRAVENÝ (session 43)** — odblokovaný `/assets/` (bol root cause slabej indexácie); ponechaný `Disallow: /kosik` |
...
| `public/pictures/graphics/web_pics/` | **NEW (session 32)** owner-supplied renders (png source + webp): `auto_hero` (hero prenájom, 1774×887), `auto_katalog` (katalóg pravý roh, cropped 1232×640), `bager` (session-32 ľavý roh, cropped 1000×664 — **orphan od s42**). Session-32 PNGs mali baked-in checkerboard bg (RGB, no alpha) → webp flattened to `#FAFAFA` + cropped. **(session 42) `bager_web`** = nový ľavý rohový bager (JCB 19c), owner PNG bol **RGBA s reálnou alfou** (nie checkerboard) → resize 1024×683 + webp q80 s alfou (177 KB); na `#FAFAFA` splýva sám (netreba flatten). To re-process: resize png → export webp preserving alpha (`method=6`) |
