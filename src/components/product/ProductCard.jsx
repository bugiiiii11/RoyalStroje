...
export default function ProductCard({ product, customerType = 'po' }) {
  ...
  return (
    <>
      <div className="group bg-white border border-zinc-200 rounded-xl md:rounded-2xl overflow-hidden hover:border-orange-primary/50 hover:shadow-md hover:shadow-orange-primary/10 shadow-sm shadow-zinc-900/5 hover:-translate-y-1 transition-all duration-300 ease-out relative">
        ...
        <div className="relative aspect-[4/3] md:aspect-square bg-gradient-to-br from-orange-100 via-orange-50 to-zinc-100 overflow-hidden">
          ...
          {/* Status Badges */}
          {product.isNew && (
            <div className="absolute top-3 left-3 px-2 py-0.5 bg-orange-primary border border-orange-primary text-white text-[10px] font-bold rounded uppercase tracking-wide shadow-lg" style={{ zIndex: 2 }}>
              Novinka
            </div>
          )}

          {product.badge && (
            <div className="absolute bottom-3 left-3 px-2 py-0.5 bg-zinc-900/90 border border-white/20 text-white text-[10px] font-bold rounded uppercase tracking-wide shadow-lg" style={{ zIndex: 2 }}>
              {product.badge}
            </div>
          )}

          {/* Price Badge - Bottom Right */}
          <div className="absolute bottom-2 right-2 md:bottom-3 md:right-3 bg-zinc-900 border border-orange-primary/40 rounded-lg px-2 py-1.5 md:px-3 md:py-2 shadow-lg shadow-black/30" style={{ zIndex: 2 }}>
        ...
        <div className="p-2 md:p-4">
          {/* Product Name */}
          <h3 className="text-sm md:text-base font-bold text-zinc-900 mb-0.5 md:mb-1 leading-tight line-clamp-1">
            {product.description}
          </h3>

          {/* Description */}
          <p className="text-zinc-500 text-xs md:text-sm mb-2 md:mb-4 leading-relaxed line-clamp-1">
            {product.name}
          </p>
          ...
          <div className="flex flex-col gap-1.5 md:gap-2">
            ...
            <Link
              to={`/${product.id}`}
              className="relative inline-flex items-center justify-center gap-1 md:gap-1.5 px-2 py-2.5 md:px-3 md:py-2.5 font-bold text-xs md:text-sm rounded-lg transition-colors overflow-hidden group/detail bg-white border border-zinc-200 text-zinc-700 hover:border-orange-primary/50 hover:text-orange-primary active:scale-[0.98] min-h-[44px]"
            >
            ...
            {product.blogArticleSlug && (
              <Link
                ...
                className="group/blog text-blue-600 font-bold text-[10px] md:text-xs flex items-center justify-center gap-1 hover:text-blue-700 transition-all py-1"
              >
          ...
          {/* Stock Status */}
          {!product.inStock && (
            <div className="mt-2 text-center text-red-600 text-[10px] font-semibold">
