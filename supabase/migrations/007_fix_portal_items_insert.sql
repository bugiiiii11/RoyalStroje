-- Fix: Allow portal (Royal Card) users to insert reservation_items for their own reservations
-- The existing "items_own" policy only allows SELECT, not INSERT
CREATE POLICY "items_own_insert" ON reservation_items
...
-- Also apply VAT fix (004) if not yet applied
